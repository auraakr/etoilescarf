<x-app-layout>
    <x-slot name="header">
        ...
        <a href="{{ route('admin.users.create') }}" class="text-body box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading focus:ring-4 focus:ring-neutral-tertiary shadow-xs font-medium rounded text-sm px-4 py-2.5 focus:outline-none">Tambah user</a>
    </x-slot>
                    ...
                    @foreach ($users as $user)
                        <tr class="bg-neutral-primary-soft border-b border-default hover:bg-neutral-secondary-medium hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @elseif($user->role === 'product_admin')
                                    <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded">Product Admin</span>
                                @elseif($user->role === 'sales_admin')
                                    <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded">Sales Admin</span>
                                @endif
                            </td>
                            <td class="px-6 py-3">
                                <div class="inline-flex rounded-base shadow-xs -space-x-px" role="group">
                                    <a href="{{ route('admin.users.edit', $user->id) }}" class="inline-flex items-center text-body bg-neutral-primary-soft border border-default hover:bg-orange-50 hover:text-orange-600 focus:ring-3 focus:ring-orange-100 font-medium leading-5 rounded-s-base text-sm px-3 py-2 focus:outline-none">
